...
export function calculateMinInterval(wilsonLower, total) {
  ...
  // A just-seen clue rests at least baseMinutes even at Wilson 0, so a clue
  // drilled in one sitting won't resurface within the same drilling burst.
  const baseMinutes = 5;
  ...
  // Factor in total attempts - more attempts extend the interval. The (0.5 +
  // wilsonLower) term means repeated practice lengthens spacing even for
  // low-Wilson clues, so a correctly-answered-a-few-times clue still rests for
  // tens of minutes rather than reappearing across back-to-back sessions.
